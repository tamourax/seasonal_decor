...
| S3 | 1.56 | 2.94 | 6.49 | 11.13 | 14.37 | 18.16 | 1.55% | 0.15% | PASS* |
...
`*` S3 uses the dedicated 5-run re-test median.
High variance on emulator; confirm on physical mid-tier device.
...
| Android | S3 | +1.23 ms | +8.79 ms | +8.96 ms | +1.39 pp |
...
Presence in this list indicates spike hotspots, not automatic threshold failure.
...
1. Scene S3
   - Timestamp window: N/A (FrameTiming summary only)
   - Thread: Raster
   - Symptom: re-run (5 runs) still shows spikes, worst frame 68.82ms
1. Scene S2
   - Timestamp window: N/A (FrameTiming summary only)
   - Thread: Raster
...
- `docs/performance/android_s3_rerun_2026-02-15.md`
